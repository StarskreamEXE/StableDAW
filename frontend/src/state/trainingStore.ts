...
export const useTrainingStore = create<TrainingStoreState>()((set, get) => ({
  ...
  refreshMetadata: async () => {
    try {
      ...
      logInfo('training', `Metadata: model=${(modelPayload as ModelInfo).active_model} autoencoders=${(aePayload.available_autoencoders ?? []).length}`);
    } catch (error) {
      ...
      logWarn('training', `Metadata load failed: ${message}`);
    }
  },
  ...
  startLoraTraining: async ({ modelName, dataDir, outputDir, rank, alpha, steps }) => {
    ...
    logInfo('training', `Submitting LoRA: model=${modelName} rank=${rank} alpha=${alpha} steps=${steps}`);
    ...
    try {
      ...
      while (true) {
        ...
        if (job.status === 'completed') {
          ...
          logInfo('training', `LoRA job completed: ${jobId.slice(0, 8)}`);
          await get().refreshJobs();
          return;
        }

        if (job.status === 'failed') {
          logError('training', `LoRA job failed: ${jobId.slice(0, 8)}`);
